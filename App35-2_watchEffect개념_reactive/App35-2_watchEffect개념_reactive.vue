...
<script>                                     
import { ref, computed, watchEffect, reactive} from 'vue'; // (1) reactive 입력
...
export default {
  ...
  setup() {
    ...
    const a = reactive({         // (2) 입력함
      b:1                        // (2) 입력함 (a안에 b가 들어있다)
    });

    watchEffect(() => {          // (3) 입력함
      console.log(a.b);          // (3) 입력함 (여기까지 하면 console에 1이 찍힌다)
    });
    a.b = 4;                     // (4) 입력함 (여기까지 하면 console에 1과 4가 찍힌다)
                                 //     (3)까지해서 1이 찍힌 것이고, 여기서 a.b를 4로 바꿔주니 바뀌었으니까
                                 //     watchEffect가 감지하고 다시 console.log(a.b)가 실행되어 4가 찍힌 것이네
